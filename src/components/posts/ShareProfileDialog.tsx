...
export default function ShareProfileDialog({ profile, open, onOpenChange }: ShareProfileDialogProps) {
  ...
  const handleShareToUser = async (user: any) => {
    ...
    try {
      ...
      const profileUrl = `${window.location.origin}/users/${profile.username}`;
      await channel.sendMessage({
        text: `Shared a profile: ${profileUrl}`,
        attachments: [
          {
            type: "share-card",
            shareType: "PROFILE",
  ...
  const handleCopyLink = () => {
    const link = `${window.location.origin}/users/${profile.username}`;
  ...
  const handleWhatsAppShare = () => {
    const text = encodeURIComponent(`Check out @${profile.username}'s profile on Next Social!`);
    const url = encodeURIComponent(`${window.location.origin}/users/${profile.username}`);
  ...
  const handleSmsShare = () => {
    const text = encodeURIComponent(`Check out @${profile.username}'s profile: ${window.location.origin}/users/${profile.username}`);
    ...
    >
      ...
      >
        ...
        <div className="relative px-5 py-3 border-b border-zinc-900 flex items-center justify-center flex-shrink-0">
          <span className="font-black text-white text-[17px] tracking-wide">Share Profile</span>
          <button 
            onClick={() => onOpenChange(false)}
